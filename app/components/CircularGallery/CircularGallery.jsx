    ...
    createShader() {
      ...
      `,
        ...
        void main() {
          ...
          vec2 localPos = (vUv - 0.5) * uPlaneSizes;
          vec2 visibleHalfSize = (0.5 / ratio) * uPlaneSizes;
          vec2 bounds = visibleHalfSize - uBorderRadius;
          float r = uBorderRadius * min(uPlaneSizes.x, uPlaneSizes.y);
          
          float d = roundedBoxSDF(localPos, bounds, r);
          
          // Smooth antialiasing for outer edges
          float edgeSmooth = 0.005;
          float alpha = 1.0 - smoothstep(-edgeSmooth, edgeSmooth, d);
          
          if (alpha < 0.1) discard;
          
          // Inner pixelated border (hard step)
          float thickness = 0.08;
          if (d > -thickness) {
              gl_FragColor = vec4(0.0, 0.0, 0.0, alpha); // explicitly black
          } else {
...
class App {
    ...
    onTouchUp() {
        this.isDown = false;
        this.onCheck();
        if (!this.dragged && this.onItemClick && this.medias && this.medias.length > 0) {
            ...
            if (clickedIndex !== -1) {
                const activeIndex = clickedIndex % this.originalLength;
                this.onItemClick(activeIndex);
            }
        }
    }
    onWheel(e) {
